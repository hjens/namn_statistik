...
def year_span_for_name(df_all: pd.DataFrame, name: str, target_fraction: float):
    """
    ...
    while f < target_fraction:
        ...
        if (
            right_year <= df_name.year.max()
            and df_name.query("year == @right_year").num.values[0]
            > df_name.query("year == @left_year").num.values[0]
        ):
...
if __name__ == "__main__":
    ...
    df_name_std.to_csv("data/names_with_year_spans.csv", index=False)
